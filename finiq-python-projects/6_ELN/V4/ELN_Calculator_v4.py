...
def main():
    ...
    tenor_months = float(input("Enter the tenor (in months): "))

    # Fetch historical stock data using yfinance
    start_date = datetime.strptime(input("Enter the Start Date (in format DD-MMM-YYYY): "), "%d-%b-%Y")
    end_date = start_date + relativedelta(months=int(tenor_months))
    ...
    plt.title(f'{ticker} Stock Price for {int(tenor_months)} months')
    plt.legend()
    plt.grid(True)
    plt.show()
